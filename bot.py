...
def parse_etd(s):
    ...
    return s.strip() + "го числа" if s.strip().isdigit() else "неизвестно"
        ...
        if line.startswith('AWB:'):
            ...
            for part in parts:
                ...
                if mt and origin in mt.group(2).upper().split('/'):
                    ...
                    break
            total += best
        # Другие сборы (Label fee, Customs и т.д.) можно добавить по аналогии
    return total

def process_offer(offer_line, fees_block, weight):
    offer_line = re.sub(r'^BY\s+', '', offer_line.strip())
    offer_line = re.sub(r'\([^)]*\)', '', offer_line)  # убираем скобки
...
async def start(update, ctx):
    await update.message.reply_text("✈️ Бот для ставок. /weight 100")

async def set_weight(update, ctx):
    try:
        w = float(ctx.args[0])
...
async def handle_message(update, ctx):
    ...
    if not offers:
        await update.message.reply_text("⚠️ Не найдены авиакомпании")
        return
    results = []
    for off in offers:
        res = process_offer(off, fees_block, w)
